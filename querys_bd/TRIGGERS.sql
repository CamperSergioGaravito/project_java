...
CREATE TRIGGER verificar_nombre 
BEFORE INSERT ON DEPARTMENTS
FOR EACH ROW  
BEGIN 
    DECLARE count_producto INT;
    SELECT COUNT(*) INTO count_producto FROM DEPARTMENTS WHERE REPLACE(LOWER(name_department),' ','') = REPLACE(LOWER(NEW.name_department),' ','');
    IF count_producto > 0 THEN 
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No puede ingresar un nombre de departamento repetido';
    END IF;
END;

-----------------------------------------------------------------------------------------------------------------------------------------------------------------

CREATE TRIGGER verificar_nombre_UPD 
